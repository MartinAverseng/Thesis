...
if b==1
    ...
    batch_size = fix(vars.batch_size*(1/a + 1/(1-b)))+1;
end
if a==0
    batch_size = 100;
end
...
while ~satisfied
    ...
    tFreqTic = tic;
    if startFreq~=0        
        rho = unique([besselJroots(0,100);besselJroots(startFreq,UpperBound)]);
        UpperBound = length(rho);
    else
        rho = besselJroots(0,UpperBound);
    end
    ...
    if a==0
        tTestLinf = linspace(0,0.1,fix(10*max(rho)*0.1)+1);
    else
    ...
    while ~cholDone && batch_size>1
        try
            ...
            warning('Cholesky failed because condition number is too high');
            disp('Restarting with new values of a and b');
