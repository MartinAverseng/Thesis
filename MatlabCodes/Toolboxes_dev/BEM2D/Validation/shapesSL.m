...
K1 = dM - k^2*(omega2 + 0.0*M*(dM + 1*M*ones(N+1))^(-1)*M-M);
K =  K1 - k^2*M;
Np = 40;
theta = pi/3;
...
% Y0 = 0.6;
% X0 = -4;
% cylindWave = R2toRfunc(@(Z)(besselh(0,k*sqrt((Z(:,1) - X0).^2 + (Z(:,2) - Y0).^2))));
% incWave = cylindWave;
...
%     while y1tmp < y2
%         y2tmp = y1tmp + step_y;
%         x = linspace(x1tmp,x2tmp,600);
%         y = linspace(y1tmp,y2tmp,600);
%         [X,Y] = meshgrid(x,y);
%         Sw.set_X([X(:) Y(:)],'a',a/10);
