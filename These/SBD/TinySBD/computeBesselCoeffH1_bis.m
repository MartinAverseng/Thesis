function [alpha,rho,w0,reMakeError] = computeBesselCoeffH1_bis(a,b,func,derivative,tolInf,askGraph,Pmax)
...
timeTotal = tic;
UpperBound = min(max(fix((-log(tolInf)+1)/a/3)+1,100),Pmax);
...
tic
resolNGL = 5;
NGL = min(resolNGL*length(rho),8000);
...
Pguess = min(fix(UpperBound*1/2)+1,Pmax);
