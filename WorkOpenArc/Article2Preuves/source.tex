...
\begin{abstract}
	This work is devoted to the theoretical and numerical analysis of some new preconditioners for the Helmholtz integral equation on open curves. 
\end{abstract}

\section*{Introduction}
 In \cite{} \toDo{Papier à venir}, two new preconditioners were introduced for the Helmholtz integral equation outside an open curve in the plane, either with Dirichlet or Neumann boundary conditions. Those preconditioners generalize the preconditioners introduced in \cite{antoine2007generalized} in the case of smooth closed curves. In the case of an open arc, solutions of the standard integral equations have edge singularities, and one is led to introduce weighted integral operators. There is a natural analytical and numerical framework that emeges with this choice of operator, for which he literature lacks general results. 
 
 In this work, we introduce two new analytical tools that allow to fully study the problem. On the first hand, we define in \autoref{sec:analyticalSetting} two family of interpolation scales $T^s$ and $U^s$ together with a class of operators enjoying some properties resembling pseudo-differential operators. This allows us to prove the main theorems of \cite{}. This is the object of section \autoref{sec:proof}. On the other hand, we analyze a simple Galerkin setting with piecewise polynomials based on a weighted $L^2$ space, and prove optimal rates of convergence. This analysis is presented in \autoref{sec:numerical}.
 
 We start by introducing the problem and the notations in the first section. 

\section{The scattering problem outside an open curve}

Let $\Gamma$ be a smooth non-intersecting open curve in $\R^2$, and let $k \geq 0$ the wave number. We seek a solution to the problem
\begin{equation}
...
We introduce the Chebyshev polynomials of first and second kinds, respectively given by
\[T_n(x) = \cos(n \arccos(x)),\]
and 
\[U_n(x) = \dfrac{\sin((n+1) \arccos(x))}{\sqrt{1 - x^2}}\,\]
for $x \in [-1,1]$. Let $\omega$ the operator $u(x) \mapsto \omega(x)u(x)$ with $\omega(x) = \sqrt{1 - x^2}$ and let $\partial_x$ the derivation operator. The Chebyshev polynomials satisfy
...
\begin{Rem}
	...
	is diverging.
\end{Rem}
Two natural derivation operators arise from our context, that give another link between $T^s$ and $U^s$. They arise from the identities 
\begin{align}
...
\end{proof}
The operator $\partial_x$ isn't continuous from $T^s$ to $T^{s-1}$. However, the following result holds:
\begin{Cor}
	\label{corDxT2T0}
...
\begin{Lem}
	There holds $L^2_\omega = \frac{1}{\omega}L^2_\frac{1}{\omega}$ and the pointwise multiplication by $\frac{1}{\omega}$ defines a bijective isometry from $L^2_\frac{1}{\omega}$ to $L^2_\omega$ with inverse $\omega$. 
	\label{isometrie}
\end{Lem}
\begin{Lem}
...
\end{proof}
\subsection{Some classes of operators on $T^s$}
\label{subsec:classesOfOp}
We now proceed to define a class of operators on $T^s$ that is stable under composition and enjoys some properties similar to pseudo-differential operators. However, we shall not look for asymptotic expansions of symbols, a result we don't need for our application. Results analog to asymptotic expansion and symbolic calculus may actually not be available for the rudimentary class we introduce here. For this reason, we refrain from using the term "pseudo-differential operators". The main motivation for this definition is \autoref{CommutPDO}. To ease the computations, we define $T_n = T_{\abs{n}}$ for $n \in \Z$. 
...
\begin{Def}
	For any real $p$, an operator $A$ belongs to the class $S^{p}$ if there exists a function $a : \N \times \Z \to \R$ satisfying: 
	\begin{itemize} 
		\item[(i)]$	\forall k \in \Z, \quad A T_k = \displaystyle\sum_{i = -\infty}^{+\infty} a(k,i)T_{k-i}$
		\item[(ii)]$	\forall \alpha \in \N, \forall  i,k  \in \Z, \quad  \abs{\Delta_k^\alpha a(k,i)} \leq C_{\alpha,i} (1 + k)^{p - \alpha}$
		\item[(iii)] There exists $N \in \N$ such that
		\[\abs{i} \geq N \implies \forall k \in \Z, \quad a(k,i) = 0.\]  
	\end{itemize}
	where $\Delta_k$ is the discrete derivation operator in the first variable, defined by
...
\end{Def}
Symbols can be viewed as semi-infinite matrix $(a(k,i))_{k \in \N, i \in \Z}$. The condition (ii) controls the decay of the coefficients along the horizontal lines $i = $ constant, while (iii) imposes that the vertical bandwidth is finite.

An awkward feature of this definition, allowed by the convention $T_{-n} = T_n$, is that the null operator admits non-trivial symbols, and thus, the symbol of an operator $A$ is not defined uniquely. The notation $\sigma_A$ only refers to one possible symbol of $A$. It seems difficult to find a natural definition both ensuring uniqueness of the symbol and expressing the same conditions as above. 
\begin{Lem}
...
\begin{proof}
	...
	\[(1+k_1)^{p - \beta} \leq C(1+k)^{p - \beta}(1 + \alpha + \abs{j})^{\abs{p - \beta}}.\]
	The same arguments applied to $b$ finally lead to 
...
\begin{proof}
	...
	\end{eqnarray*}
 	We show that asumption (ii) holds for one of the terms of the first sum when $j$ is positive. The other cases are treated analogously. We can write 
\begin{eqnarray*}
