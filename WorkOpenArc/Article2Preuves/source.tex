...
On the family of spaces $H^s_p(0,T)$, a class of periodic pseudo-differential operators has been introduced in \cite{thrunen1998symbol}, with symbolic calculus. A periodic pseudo-differential operator on $H^s_p(0,T)$ of order $p$ is an operator of the form 
...
Here, $\hat{u}_n = \frac{1}{T}\int_{0}^{T}u(t) e^{-i\frac{2n\pi t}{T}}dt$ are the usual Fourier coefficients of $u$ and $\partial_t \isdef \frac{1}{iT}\frac{\partial}{\partial t}$, $\partial_\xi \isdef \frac{1}{iT}\frac{\partial}{\partial \xi}$. 
The class of symbols that satisfy this condition is denoted by $\Sigma^p$, and $\Sigma^{-\infty} \isdef \Cup_{p \in \Z} \Sigma^p$. The operator corresponding to a symbol $\sigma$ is denoted by $\textit{Op}(\sigma)$ and thus, the set of periodic pseudo-differential operators is $\textit{Op}(\Sigma^p)$. 
...
An operator in $\textit{Op}(\Sigma^p)$ maps continuously $H_p^s(0,T)$ to $H_p^{s + p}$ for all $s \in \R$. The composition of two operators in $\textit{Op}(\sigma^p)$ and $\textit{Op}(\sigma^q)$ gives rise to an operator in $\textit{Op}(\sigma^{p+q})$. 
