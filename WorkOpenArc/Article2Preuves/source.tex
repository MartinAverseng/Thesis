...
\end{proof}
Two natural derivation operators arise from our context, that give another link between $T^s$ and $U^s$. 
\begin{Lem}
	\label{derivations}
	For all real $s$, the operator $\partial_x$ can be extended into a continuous map from $T^{s+1}$ to $U^{s}$ defined as 
	\[ \duality{\partial_x u}{v}_{\omega} \isdef -\duality{u}{\omega \partial_x \omega v}_{\frac{1}{\omega}} \quad \forall v \in U^{\infty}.\] 
...
\end{Lem}
\begin{proof}
	The result is related to the two identites
	\[T_n' = n U_{n-1} \quad \omega \partial_x \omega U_n = -(n+1)T_{n+1}.\]
	The first one is obtained for example from the trigonometric definition of $T_n$. This combined with $-(\omega \partial_x)^2 T_n = n^2 T_n$ gives the second identity. Now, let $u \in T^{s+1}$ for some $s \in \R$, with 
	\[ u = \sum_{n = 0}^{+ \infty} \hat{u}_n T_n.\]
	When $u$ is smooth, the coefficients $\hat{u}_n$ have a fast decay and thus,  $\partial_x u$ is just given by term by term differentiation
	\[\partial_x u = \sum_{n=0}^{+ \infty} n \hat{u}_n U_{n-1}.\]
	If we fix $v \in U^{\infty}$, we then have
	\[\duality{\partial_x u}{v}_\omega = \sum_{n = 0}^{+ \infty} (n+1)\hat{u}_{n+1} \check{v}_n.\]
	Since $\omega \partial_x \omega v = -\sum_{n=0}^{+ \infty}(n+1) \check{v}_{n} T_{n+1}$, we indeed have 
	\[\duality{\partial_x u}{v}_\omega = -\duality{u}{\omega \partial_x \omega v}_\frac{1}{\omega}.\]
	The rhs still has a sense when $u$ is not differentiable, because if $v \in U^{\infty}$, then $v$ is $C^\infty$ and $\omega \partial_x \omega v = (1-x^2)v' - xv$ also lies in $C^{\infty} = U^\infty$. This thus provides a definition, in a weak sense, of the derivative of $u$. We just have to check that the announced continuity. For all $n$, we have $\duality{\partial_x u}{U_n}_\omega = n\hat{u}_{n-1}$
	The definition of $\partial_x$ is thus correct. The claimed continuity  is an immediate consequence of 
