...
complete study of such operators is postponed to the second part of this work \todo{Sinon on met ça ?}.
\end{abstract}
...
\cite{antoine2007generalized}. The approach proves to be very effective, leading to optimal convergence rate in terms of the mesh-step, \toDo{providing that the latter be mildly refined near the edge in a way that is also described precisely}. On the other hand, proofs of convergence rates and approximation 
...
for Laplace problems, are nevertheless \toDo{included here}.
...
\todo{J'enlève le "Clearly", le résultat n'est pas trivial mais montré dans mclean}
...
$\tilde{H}^{-1/2}(\Gamma)$ to $H^{1/2}(\Gamma)$ \cite[Theorem 1.8]{wendland1990hypersingular}. \todo{J'ai fait une autre phrase, sinon on peut croire que la phrase porte sur l'opérateur de trace}
...
we observe a very slight increase with respect to the wavenumber. \todo{en maillage à fréquence fixée, c'est bien indépentdant donc j'ai remplacé with respect to "mesh size" par "wavenumber"}
\begin{table}[H]
	\begin{center}
		\begin{tabular}{m{4em} | m{4em} | m{4em} | m{4em} | m{4em}} 
			\hline
